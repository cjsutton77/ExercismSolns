...
heaven::Vessel::Vessel(std::string _name, int _number):
    ...
    {};
auto heaven::Vessel::replicate(std::string newName){
    return heaven::Vessel(newName,2);
}
void heaven::Vessel::make_buster(void){
    this->busters = 1;
}
auto heaven::Vessel::shoot_buster(void){
    if (busters == true){
        busters = false;
        return true;
    } 
...
auto heaven::get_older_bob(heaven::Vessel a, heaven::Vessel b){
    if (a.generation < b.generation) return a.name;
    else return b.name;
}

auto heaven::in_the_same_system(heaven::Vessel a, heaven::Vessel b){
